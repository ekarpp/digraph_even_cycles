...
int Solver::shortest_even_cycle(Graph &G) const
{
    ...
    const Polynomial p = util::poly_interpolation(gamma, delta);
 ...
 * cycle exists */
int Solver::shortest_even_cycle_brute(const Graph &G) const
{
    int len = G.get_n() + 1;
